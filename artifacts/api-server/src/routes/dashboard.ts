    ...
    const [
      ...
      pool.query(
        `SELECT COUNT(*) as qtd, COALESCE(SUM(total), 0) as soma
         FROM vendas WHERE created_at >= $1`,
        [inicioDia]
      ),
      pool.query(
        `SELECT COUNT(DISTINCT cliente_id) as clientes,
                COALESCE(SUM(total - COALESCE(valor_pago, 0)), 0) as soma
         FROM vendas WHERE status IN ('fiado', 'fiado_atrasado')`
      ),
      pool.query(
        `SELECT COALESCE(SUM(total), 0) as soma
         FROM vendas WHERE status = 'confirmada' AND created_at >= $1`,
        [inicioMes]
      ),
      pool.query(`SELECT COUNT(*) as qtd FROM clientes`),
      pool.query(
      ...
      pool.query(
        `SELECT c.nome,
                SUM(v.total - COALESCE(v.valor_pago, 0)) as em_aberto,
                MIN(v.created_at) as primeira_data
         FROM vendas v
         JOIN clientes c ON c.id = v.cliente_id
         WHERE v.status IN ('fiado', 'fiado_atrasado')
         GROUP BY c.id, c.nome
         ORDER BY em_aberto DESC LIMIT 5`
      ),
    ]);

    res.json({
      vendasHoje: {
        qtd: parseInt(vendasHoje.rows[0].qtd),
        soma: parseFloat(vendasHoje.rows[0].soma),
      },
      fiadosAbertos: {
        clientes: parseInt(fiadosAbertos.rows[0].clientes),
        soma: parseFloat(fiadosAbertos.rows[0].soma),
      },
